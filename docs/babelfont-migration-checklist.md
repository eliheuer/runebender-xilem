...
| M12 adapters and constructors | Shared canonical new-font and in-memory UFO constructors are integrated; the browser and native New Font paths now open canonical Projects, `f33e193` stages complete UFO/Designspace saves before publication while preserving exact custom GLIF paths and opaque filesystem payloads, external feature dependencies participate in native conflict detection, and the reviewed source-format allowlist is enforced by whole-UFO and Designspace regressions. | Move the remaining legacy CLI caller. |
