...
| M06 application cutover | Whole-glyph/component edits, composition publication and CLI glyph inspection now use canonical Project APIs through `efa4362`; 170 enabled binary tests are present. | Replace the transitional Session glyph rebase, mixed legacy history and remaining canvas, clipboard and panel readers. |
...
| M08/M09 pipeline | Canonical interpolation, source construction, compiler metadata, Designspace structure, typed special rendering and typed HOI ownership are integrated; `5e9d2f1` closes sparse-source and instance structural transactions and the combined variable-project suite passes 63 tests. | Finish compiler/export acceptance and remaining application callers. |
...
| M12 adapters and constructors | Active in the reassigned constructor lane with shared new-font and in-memory UFO import boundaries under test. | Integrate the reviewed constructor/import commit and move browser bootstrap plus native reload callers. |
