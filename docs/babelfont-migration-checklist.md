...
| M06 application cutover | FontModel whole-glyph operations plus component add and alignment now commit through canonical Project transactions in `2e16e09`; 166 enabled binary tests pass. | Replace the transitional Session glyph rebase, mixed legacy history and remaining canvas, clipboard, compose and CLI readers. |
...
| M08/M09 pipeline | Canonical interpolation, source construction, compiler metadata, Designspace structure, typed special rendering and typed HOI ownership are integrated through `4419585`; the combined variable-project suite passes 63 tests. | Finish compiler/export acceptance, remaining callers and the mapped/sparse source matrix. |
| M10 proposals and versions | Composition publication and selective proposal/version installation are direct guarded canonical transactions through `7718d1c`; no internal proposal edit round-trip remains. | Move the remaining application and headless composition callers, then remove superseded compatibility code during M13. |
