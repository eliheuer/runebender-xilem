...
The focused UFO boundary test passed one test, glyph metadata passed four tests, and the existing glyph-operation regression group passed eighteen tests.
...
Hand `b814d6c` to the lead for selective integration and adjust only the small module path or accessor names it requests.
Then add independently testable canonical glyph metadata boundary operations for Unicode, notes, export and category while the lead wires source storage and rename.
