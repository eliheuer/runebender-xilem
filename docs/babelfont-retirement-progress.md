...
| `application/editor/inspector.rs:713-714` overview marks | `record_undo` plus `Master::edit_glyph`. | Use `LayerEditDraft::set_mark_color` in one transaction per selected layer and retain the existing overview batch ordering. |
