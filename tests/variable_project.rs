...
#[test]
fn canonical_layer_transaction_reconciles_detached_ufo_algorithms() {
    let (_scratch, mut project, _fonts) = adversarial_fixture();
    let layer = project
        .document_source(SourceId(0))
        .unwrap()
    ...
    let before = project.document_layer("A", &address.layer).unwrap();
    let point_id = before
        .contours()
        .next()
        .unwrap()
        ...
        .unwrap()
        .id();
    let original_width = before.width();

    let mut transaction = project.begin_document_layer_transaction(&address).unwrap();
    let mut glyph = transaction.compatibility_glyph();
    assert!(!transaction.reconcile_compatibility_glyph(&glyph).unwrap());
    glyph.width = original_width + 19.25;
    glyph.contours[0].points[0].x += 7.5;
    assert!(transaction.reconcile_compatibility_glyph(&glyph).unwrap());
    let draft = transaction.draft().view();
    assert_eq!(draft.width(), original_width + 19.25);
    assert_eq!(
        draft
            .contours()
            .next()
            ...
            .id(),
        point_id,
        "compatibility reconciliation replaced a matched point identity"
    );

    let snapshot = transaction.compatibility_glyph();
    let mut invalid = snapshot.clone();
    invalid.components[0].transform.x_offset = f64::INFINITY;
    assert_eq!(
        transaction.reconcile_compatibility_glyph(&invalid),
        Err(runebender::document::DocumentEditError::NonFinite)
    );
    assert_eq!(transaction.compatibility_glyph(), snapshot);
    ...
    assert_eq!(committed.width(), original_width + 19.25);
    assert_eq!(
        committed
            .contours()
            .next()
