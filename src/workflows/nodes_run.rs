...
fn run_node(
    ...
    match node_type.name.as_str() {
        ...
        "core.features" => {
            ...
            let generated = features::generate_project(&project, source_id)
                .ok_or_else(|| format!("{}: source is not in the document", source.display()))?;
        ...
        "core.compose" => {
            ...
            let plan = crate::font::compose::plan_project(
                &project,
                source_id,
            ...
            out.insert(
                ...
                RunValue::Layer {
                    source: source.to_path_buf(),
                    name: proposal::layer_name(crate::font::compose::TASK),
                },
            );
...
<designspace format="5.0">
  ...
  <sources>
    ...
    fn compose_node_writes_only_a_canonical_proposal() {
        ...
        assert_eq!(
            proposal::find_project(&project, source, crate::font::compose::TASK)
                .unwrap()
                .glyphs,
            ["Aacute"]
        );
