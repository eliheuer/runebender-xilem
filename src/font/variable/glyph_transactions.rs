...
impl CanonicalSourceStructureSnapshot {
    pub(in crate::font) fn has_layer(&self, name: &str, layer: &super::LayerId) -> bool {
        self.glyphs
            .get(name)
            .is_some_and(|glyph| glyph.layers.contains_key(layer))
    }

    pub(in crate::font) fn font_metadata(
        &self,
        source: super::SourceId,
    ) -> Option<&crate::font::canonical_metadata::CanonicalFontMetadata> {
        Some(&self.source_metadata.get(&source)?.font_metadata)
    }

    pub(in crate::font) fn add_empty_glyph(
        &mut self,
        name: &str,
        width: f64,
        codepoint: Option<char>,
    ) -> Result<Vec<GlyphLayerAddress>, String> {
        crate::font::canonical_metadata::validate_name(name).map_err(|error| error.to_string())?;
        if !width.is_finite() {
        ...
        for (source, id) in default_layers {
            ...
            glyph.source_metadata.insert(
                source,
                crate::font::model::glyph_metadata::CanonicalSourceGlyphMetadata::default(),
            );
            let geometry = self
                .glyph_geometry
                .get_mut(name)
    ...
    pub(in crate::font) fn duplicate_glyph(
        &mut self,
        source: &str,
        name: &str,
    ) -> Result<Vec<GlyphLayerAddress>, String> {
            ...
            let (id, preserved) = source_glyph
                ...
                .find(|(id, _)| {
                    source_layer.id.as_deref()
                        == Some(crate::font::babelfont::layer_key(id).as_str())
                })
                .ok_or("glyph layer geometry has no preservation payload")?;
            let (layer, preserved) = crate::font::babelfont::glyph_transactions::duplicate_layer(
                source_layer,
                preserved,
                id,
                name,
    ...
    pub(in crate::font) fn remove_glyph(
        &mut self,
        name: &str,
    ) -> Result<Vec<GlyphLayerAddress>, String> {
        let glyph = self
    ...
    pub(in crate::font) fn rename_glyph(
        &mut self,
        old: &str,
        new: &str,
    ) -> Result<Vec<GlyphLayerAddress>, String> {
        ...
        for preserved in glyph.layers.values_mut() {
            crate::font::babelfont::glyph_transactions::rename_layer(preserved, new);
        }
        for geometry in &mut self.glyph_geometry.0 {
            ...
            for layer in &mut geometry.layers {
                ...
                crate::font::babelfont::glyph_transactions::rename_references(
        ...
        for layer in &mut geometry.layers {
            let Some((_, preserved)) = glyph.layers.iter_mut().find(|(id, _)| {
                layer.id.as_deref() == Some(crate::font::babelfont::layer_key(id).as_str())
            }) else {
                continue;
            };
            crate::font::babelfont::glyph_transactions::rename_references(
