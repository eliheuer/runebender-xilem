    ...
    pub(crate) fn try_interpolated_layer_at(
        ...
        if let (Some(axis), Some((lo, hi))) = (axes.first(), self.axis_end_sources()) {
            ...
            if let Some(curves) = curves.filter(|curves| !curves.is_empty()) {
                ...
                let design = crate::font::var_model::denormalize_value(
                    normalized,
                    axis.design_minimum(),
                    axis.design_default(),
                    axis.design_maximum(),
    ...
    pub fn trajectory_samples(
        ...
        for step in 0..=steps {
            ...
            location.insert(
                ...
                crate::font::var_model::normalize_value(
                    design,
                    axis.design_minimum(),
                    axis.design_default(),
                    axis.design_maximum(),
    ...
    pub fn rule_substitute(&self, glyph_name: &str) -> Option<String> {
        ...
        let design: HashMap<_, _> = designspace
            ...
            .map(|axis| {
                ...
                (
                    axis.id(),
                    crate::font::var_model::denormalize_value(
                        normalized,
                        axis.design_minimum(),
                        axis.design_default(),
                        axis.design_maximum(),
...
mod tests {
    ...
    use crate::font::model::hoi::HoiIntermediates;
    ...
    fn brace_layer_refines_interpolation() {
        ...
        let loc_500 = {
            ...
            l.insert(
                axis.name.clone(),
                crate::font::var_model::normalize_value(500.0, axis.min, axis.default, axis.max),
            );
            l
        };
    ...
    fn hoi_preview_is_exact_without_baking() {
        ...
        let at = |project: &Project, design: f64| {
            ...
            location.insert(
                axis.name.clone(),
                crate::font::var_model::normalize_value(design, axis.min, axis.default, axis.max),
            );
    ...
    fn trajectories_sample_the_axis_and_bend_with_braces() {
        ...
        loc.insert(
            axis.name.clone(),
            crate::font::var_model::normalize_value(550.0, axis.min, axis.default, axis.max),
        );
        let source = project.source_id(0).unwrap();
        let default = project.document_source(source).unwrap().default_layer();
    ...
    fn rule_substitute_switches_past_the_condition() {
        ...
        let identities = project
            ...
            .collect::<Vec<_>>();
        let canonical = crate::font::model::designspace::CanonicalDesignspace::from_norad_with_source_identities(
                &doc, identities,
            )
            .unwrap();
