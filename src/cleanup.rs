...
pub fn add_extreme_points(glyph: &mut norad::Glyph, selected: &HashSet<(usize, usize)>) -> bool {
    ...
    'outer: loop {
        ...
        for hit in crate::segment_ops::segments(glyph) {
            ...
            for t in cubic.extrema() {
                ...
                if crate::segment_ops::insert_point_on_segment(glyph, &hit, t).is_some() {
                    changed = true;
                    continue 'outer;
                }
            }
...
pub fn correct_path_directions(glyph: &mut norad::Glyph) -> usize {
    ...
    if !flip.is_empty() {
        crate::glyph_ops::reverse_contours(glyph, &flip);
    }
    flipped
}
...
mod tests {
    ...
    fn tidy_correct_and_round_fix_a_messy_glyph() {
        ...
        use kurbo::Shape as _;
        let outer = crate::glyph_paths::contour_to_bezpath(&glyph.contours[0]);
        let hole = crate::glyph_paths::contour_to_bezpath(&glyph.contours[1]);
