...
use crate::binary_import::import_binary_font;
use crate::glyph_ops::{self as ops, CurveOp, GlyphSnapshot};
use crate::lib_keys::{hoi_quad_at, read_hoi_intermediates};
use crate::theme_oklch::{self, Theme};
use crate::var_model::{Location, VariationModel};
...
pub fn extract_points(glyph: &norad::Glyph) -> Vec<GlyphPoint> {
    glyph
        ...
        .enumerate()
        .flat_map(|(ci, c)| {
            let hyper = crate::model::workspace::norad_contour_is_hyper(c);
...
impl Master {
    ...
    pub fn from_font(font: norad::Font, source_path: PathBuf) -> Self {
        ...
        let mut glyphs: Vec<GlyphEntry> = font
            ...
            .iter()
            .map(|glyph| {
                let path = Arc::new(crate::glyph_paths::glyph_to_bezpath(glyph, &font));
                GlyphEntry {
                    ...
                    path: path.clone(),
                    contour_path: Arc::new(crate::glyph_paths::contours_to_bezpath(glyph)),
                    component_path: Arc::new(crate::glyph_paths::components_to_bezpath(
                        glyph, &font,
                    )),
    ...
    pub fn rebuild_entry(&mut self, glyph_index: usize) {
        ...
        let path = Arc::new(crate::glyph_paths::glyph_to_bezpath(glyph, &self.font));
        let contour_path = Arc::new(crate::glyph_paths::contours_to_bezpath(glyph));
        let component_path = Arc::new(crate::glyph_paths::components_to_bezpath(glyph, &self.font));
    ...
    pub fn start_hyper_contour(&mut self, glyph_index: usize, x: f64, y: f64) -> Option<usize> {
        self.edit_glyph(glyph_index, |g| {
            crate::glyph_ops::start_hyper_contour(g, x, y)
        })
    }
    ...
    pub fn append_hyper_point(
        ...
        self.edit_glyph(glyph_index, |g| {
            crate::glyph_ops::append_hyper_point(g, contour, x, y, corner)
        });
    }
    ...
    /// Delete points (see `crate::glyph_ops`).
...
impl Project {
    ...
    pub fn new_font(path: PathBuf) -> Self {
        let font = crate::new_font::new_font("Untitled", "Regular", 400);
        ...
        if let Some(kind @ (GlyphsSource::File | GlyphsSource::Package)) = glyphs_ext {
            ...
            let result = match kind {
                GlyphsSource::Package => {
                    ...
                    crate::glyphs_import::glyphs_package_to_ufo_files(&entries)?
                }
                _ => {
                    let text = std::fs::read_to_string(path).map_err(|e| format!("{e}"))?;
                    crate::glyphs_import::glyphs_to_ufo_files(&text)?
                }
            };
            let stem = path
                .file_stem()
    ...
    ) -> Result<Self, String> {
        {
            ...
            let normalize_loc = |dims: &[norad::designspace::Dimension]| {
                ...
                for axis in &axes {
                    ...
                    location.insert(
                        axis.name.clone(),
                        crate::var_model::normalize_value(raw, axis.min, axis.default, axis.max),
                    );
                }
                location
            };
    ...
    pub fn interpolated_glyph(&self, glyph_name: &str) -> Option<(BezPath, f64)> {
        ...
        Some((
            crate::glyph_paths::glyph_to_bezpath(&glyph, &base.font),
            advance,
        ))
    }
    ...
    pub fn interpolated_at(&self, glyph_name: &str, location: &Location) -> Option<norad::Glyph> {
        ...
        if let (Some(axis), Some((lo, hi))) = (self.axes.first(), self.axis_end_masters()) {
            ...
            if !curves.is_empty() {
                ...
                let design = crate::var_model::denormalize_value(
                    normalized,
                    axis.min,
                    axis.default,
                    axis.max,
    ...
    pub fn trajectory_samples(
        ...
        for step in 0..=steps {
            ...
            location.insert(
                axis.name.clone(),
                crate::var_model::normalize_value(design, axis.min, axis.default, axis.max),
            );
    ...
    pub fn rule_substitute(&self, glyph_name: &str) -> Option<String> {
        ...
        let design: HashMap<&str, f64> = self
            ...
            .map(|axis| {
                ...
                (
                    axis.name.as_str(),
                    crate::var_model::denormalize_value(
                        normalized,
                        axis.min,
                        axis.default,
                        axis.max,
...
mod tests {
    use super::*;
    use crate::glyph_ops as ops;
    use crate::lib_keys::write_hoi_intermediates;
    use crate::measure::joining_band;
    use crate::metrics_keys::{read_metrics_key, write_metrics_key};
    ...
    fn brace_layer_refines_interpolation() {
        ...
        let loc_500 = {
            ...
            let mut l = crate::var_model::Location::new();
            l.insert(
                axis.name.clone(),
                crate::var_model::normalize_value(500.0, axis.min, axis.default, axis.max),
            );
            l
        };
    ...
    fn joining_bands_measure_the_connecting_stroke() {
        ...
        let path = crate::glyph_paths::contour_to_bezpath(&glyph.contours[0]);
        ...
        let moved = crate::glyph_paths::contour_to_bezpath(&glyph.contours[0]);
        ...
        if let Some(g) = font.font.get_glyph("beh-ar.medi") {
            ...
            let outline = crate::glyph_paths::glyph_to_bezpath(g, &font.font);
            assert!(
    ...
    fn hoi_preview_is_exact_without_baking() {
        ...
        let at = |project: &Project, design: f64| {
            let mut location = crate::var_model::Location::new();
            location.insert(
                axis.name.clone(),
                crate::var_model::normalize_value(design, axis.min, axis.default, axis.max),
            );
    ...
    fn trajectories_sample_the_axis_and_bend_with_braces() {
        ...
        project.masters[0]
            ...
            .unwrap()
            .insert_glyph(frozen);
        let mut loc = crate::var_model::Location::new();
        loc.insert(
            axis.name.clone(),
            crate::var_model::normalize_value(550.0, axis.min, axis.default, axis.max),
        );
        project.brace.push(BraceSource {
            master: 0,
            layer: "{550}".into(),
    ...
    fn measures_reference_stems() {
        ...
        use crate::model::workspace::Contour as WContour;
        ...
        let paths: Vec<crate::path::Path> = g
            .contours
            .iter()
            .map(|c| crate::path::Path::from_contour(&WContour::from_norad(c)))
            .collect();
