...
impl EditorWidget {
    ...
    ///
    /// Runs the img2bez tracing pipeline on the background image's
    /// source file, converting the bitmap into cubic bezier contours.
    /// The traced paths replace the current session paths and the
    /// advance width is updated from the trace result.
    fn handle_trace_image(
        ...
        if !matches!(key, Key::Character(c) if c.eq_ignore_ascii_case("t")) {
            return false;
        }
        ...
        let source_path = bg.source_path.clone();
        ...
        match img2bez::trace(&source_path, &config) {
            Ok(result) => {
                tracing::info!(
                    "Traced {} contours from {}",
                    result.paths.len(),
                    source_path.display()
                );

                // Convert img2bez BezPaths (kurbo 0.13) to
                // local kurbo BezPaths (0.12)
                ...
                // Align traced contours with the background
                // image. img2bez repositions paths to sit at
                // y=0 with LSB padding, but we need them to
                // overlay the image at its current design-space
                // position.
                use kurbo::Shape;
                ...
                {
                    let tcx = (traced_bbox.x0
                        + traced_bbox.x1)
                        / 2.0;
                    let tcy = (traced_bbox.y0
                        + traced_bbox.y1)
                        / 2.0;
                    let icx = (image_bounds.x0
                        + image_bounds.x1)
                        / 2.0;
                    let icy = (image_bounds.y0
                        + image_bounds.y1)
                        / 2.0;
                    let shift = kurbo::Affine::translate((
                        icx - tcx,
                        icy - tcy,
                    ));
                ...
                let new_paths: Vec<crate::path::Path> =
                    local_paths
                        .iter()
                        .map(|bp| {
                            crate::path::Path::Cubic(
                                bezpath_to_cubic(bp),
                            )
                        })
                        .collect();

                // Replace session paths with traced paths
                self.session.paths =
                    std::sync::Arc::new(new_paths);

                // Update advance width from trace result
