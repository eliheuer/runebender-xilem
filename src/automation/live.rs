...
use super::{agent, script_recipe};
use crate::font::{
    LayerView,
    canonical_metadata::{KerningParticipant, KerningSide},
    edit_batch, experiments,
    project::Project,
    proposal,
    variable::SourceId,
};
...
pub fn system_prompt(tools: &[agent::Tool]) -> String {
    ...
    prompt.push_str(script_recipe::AUTHORING_INSTRUCTIONS);
    prompt.push_str("\n\n");
    for tool in tools {
...
fn handle(project: &mut Project, name: &str, args: &Value) -> Result<Value, String> {
    ...
    if name == "experiment_list" {
        return Ok(experiments::list(project));
    }
    if name == "experiment_undo_apply" {
        let (source, names) = experiments::undo_apply(project)?;
        return Ok(
    ...
    if name == "experiment_fork" {
        ...
        let parent = object
            ...
            .transpose()?;
        experiments::fork(project, source, name, parent, reason)?;
        return Ok(
    ...
    if name == "experiment_apply" {
        ...
        let installed = experiments::apply(project, branch, &names, kerning, keep)?;
        return Ok(
    ...
    let mut result = match name {
        "specimen" => {
            ...
            json!({"ok":true,"scene":scene,"text":text,"kerning_revision":experiments::kerning_revision(metadata)?})
        }
        "read_kerning" => {
            let metadata = match branch {
                Some(branch) => project
            ...
            json!({"ok":true,"revision":experiments::kerning_revision(metadata)?,"pairs":metadata.raw_kerning(),"groups":metadata.groups()})
        }
        "experiment_kern" => {
            ...
            let version = project
                ...
                .get(branch)
                .ok_or("unknown experiment")?;
            if revision != experiments::kerning_revision(version.font_metadata())? {
            ...
            json!({"ok":true,"revision":experiments::kerning_revision(version.font_metadata())?})
        }
        "font_info" => {
        ...
        "glyph_inventory" => {
            ...
            let layers: Vec<(String, LayerView<'_>)> = match branch {
                Some(branch) => {
                    let experiment = project
                        .experiments
                        .versions
...
mod tests {
    ...
    fn experiment_specimen_reports_the_snapshot_kerning_revision() {
        ...
        assert_ne!(
            ...
            experiments::kerning_revision(project.document_font_metadata(source).unwrap(),)
                .unwrap(),
    ...
    fn inventory_finds_unicode_and_marks_without_changing_font() {
        ...
        project
            .edit_document_layer("eight", &layer, |draft| {
                draft.set_mark(
                    Some("green"),
                    Some(crate::font::model::glyph_metadata::MarkColor {
                        red: 0.1,
                        green: 0.8,
                        blue: 0.2,
                        alpha: 1.0,
