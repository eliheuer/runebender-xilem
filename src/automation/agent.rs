...
//! outside MCP client reads the same definitions, so the two
...
pub fn tools() -> Vec<Tool> {
    ...
    tools.push(Tool {
        ...
        parameters: serde_json::to_value(schemars::schema_for!(crate::font::edit_batch::EditBatch)).expect("schema serializes"),
    });
