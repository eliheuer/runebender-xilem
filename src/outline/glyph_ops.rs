...
pub fn contour_is_hyper(glyph: &Glyph, contour: usize) -> bool {
    glyph
        .contours
        .get(contour)
        .map(crate::outline::path::hyper_model::norad_contour_is_hyper)
        .unwrap_or(false)
}
...
pub fn convert_hyper_to_cubic(glyph: &mut Glyph, selected: &HashSet<(usize, usize)>) -> bool {
    ...
    for (ci, contour) in glyph.contours.iter_mut().enumerate() {
        ...
        let path = crate::outline::path::Path::from_contour(&ws);
        let crate::outline::path::Path::Hyper(hyper) = path else {
            continue;
        };
        let cubic = crate::outline::path::Path::Cubic(hyper.to_cubic());
...
mod tests {
    ...
    fn hyper_pen_builds_closed_solver_contour() {
        ...
        let ws = crate::outline::path::hyper_model::Contour::from_norad(&g.contours[c]);
        let back = ws.to_norad();
        assert!(crate::outline::path::hyper_model::norad_contour_is_hyper(
            &back
        ));
    }
